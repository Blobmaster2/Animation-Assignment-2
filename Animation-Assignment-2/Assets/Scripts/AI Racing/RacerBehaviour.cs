...
public class RacerBehaviour : MonoBehaviour
{
    ...
    public float veerChance = 10;

    bool updatedVelocity;
    bool isVeering = false;
    public bool catmullEnabled = true;
    bool veerRight;
    ...
    private void UpdateCatmullTrack()
    {
        ...
        if (!updatedVelocity)
        {
            ...
            {
                float distanceMult;
                if (Vector3.Distance(catmullVec, currentPos) > 4) distanceMult = 4;
                else distanceMult = Vector3.Distance(catmullVec, currentPos);
                rb.AddForce((catmullVec - currentPos).normalized * speed * distanceMult * speedMultiplier);
            }
    ...
    void Veer()
    {
        Vector3 dir = veerRight ? Vector3.right : Vector3.left;
    ...
    IEnumerator WaitForVeer()
    {
        ...
        if (Random.Range(0, 101) <= veerChance)
        {
            yield return StartCoroutine(WaitForStopVeer());
        }
    ...
    {
        veerRight = Random.Range(0, 2) != 0;
        isVeering = true;
        catmullEnabled = false;
        maxVeerTime = Random.Range(0.2f, 1);
    ...
    void SeekPowerup(Collider powerup)
    {
        ...
        rb.AddForce(direction * speed * distance);
    }

    void EvadeRacers(Collider racer)
    {
        Vector3 direction = (transform.position - racer.transform.position).normalized;
        rb.AddForce(direction * (speed * speedMultiplier) / Vector3.Distance(transform.position, racer.transform.position));
    }
    ...
    {
        StartCoroutine(TempIncreaseSpeed());
    }

    public IEnumerator TempIncreaseSpeed()
    {
        speedMultiplier = 2f;
